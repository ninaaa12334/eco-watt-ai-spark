      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 w-full grid lg:grid-cols-2 gap-12 items-center relative z-10">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
          ...
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-5">
            <span className="gradient-text">{h.tagline1[lang]}</span>
            <br />
            <span className="gradient-text">{h.tagline2[lang]}</span>
            <br />
            <span className="text-foreground">{h.tagline3[lang]}</span>
          </h1>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { icon: TrendingDown, value: 18, suffix: "%", label: h.stat1[lang] },
              { icon: DollarSign, prefix: "€", value: 14, label: h.stat2[lang] },
              { icon: Shield, value: 94, suffix: "%", label: h.stat3[lang] },
            ].map((stat, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 + i * 0.15, duration: 0.5 }} className="text-center">
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="relative">
          <div className="glass-card p-6 rounded-2xl animate-float">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-semibold text-foreground">{h.dashTitle[lang]}</h3>
              <span className="text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary font-medium">{h.live[lang]}</span>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: h.wasteFound[lang], value: "3.2 kWh", color: "bg-destructive/10 text-destructive" },
                { label: h.potentialSavings[lang], value: "€9.40", color: "bg-eco-success/10 text-eco-success" },
                { label: h.energyScore[lang], value: "78/100", color: "bg-primary/10 text-primary" },
              ].map((item) => (
            ...
            <div className="space-y-2 mb-4">
              {[
                { name: h.uploadBill[lang], icon: Camera, status: "ready" },
                { name: h.scanAppliances[lang], icon: Camera, status: "ready" },
                { name: h.billVerification[lang], icon: FileText, status: "active" },
              ].map((d) => (
            ...
            <div className="p-3 rounded-xl border border-eco-warning/30 bg-eco-warning/5 flex items-start gap-2.5">
              ...
              <div className="text-xs text-foreground">
                <span className="font-semibold">AI Insight:</span> {h.aiInsight[lang]}
              </div>
            </div>
          </div>
        </motion.div>
