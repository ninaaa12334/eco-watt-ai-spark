...
import { Zap, TrendingDown, Clock, DollarSign, Activity, Wifi, Thermometer, Lightbulb } from "lucide-react";
import AnimatedCounter from "./AnimatedCounter";

const miniDevices = [
  { name: "Living Room AC", power: "1.2 kW", status: "active", icon: Thermometer },
  { name: "Kitchen Lights", power: "0.08 kW", status: "idle", icon: Lightbulb },
  { name: "TV Standby", power: "0.02 kW", status: "waste", icon: Activity },
  { name: "Smart Speaker", power: "0.01 kW", status: "active", icon: Wifi },
];
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 w-full grid lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        >
          ...
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-5">
            <span className="gradient-text">AI that detects,</span>
            <br />
            <span className="gradient-text">controls, and reduces</span>
            <br />
            <span className="text-foreground">household electricity waste.</span>
          </h1>

          <p className="text-muted-foreground text-lg md:text-xl leading-relaxed mb-8 max-w-xl">
            EcoWatt AI monitors home energy use in real time, finds hidden waste, automates smart actions, and helps families save electricity, lower bills, and live more sustainably.
          </p>

          <div className="flex flex-wrap gap-4 mb-10">
            <a href="#demo" className="btn-primary-eco">
              <Zap className="w-4 h-4" /> Launch Demo
            </a>
            <a href="#how-it-works" className="btn-outline-eco">
              See How It Works
            </a>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { icon: DollarSign, prefix: "€", value: 14, label: "Monthly savings" },
              { icon: Clock, value: 27, label: "Unnecessary device-hours prevented" },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
        ...
        >
          <div className="glass-card p-6 rounded-2xl animate-float">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-semibold text-foreground">Energy Dashboard</h3>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: "Today's Usage", value: "12.4 kWh", color: "bg-primary/10 text-primary" },
                { label: "Waste Found", value: "1.8 kWh", color: "bg-destructive/10 text-destructive" },
                { label: "Savings", value: "€0.62", color: "bg-eco-success/10 text-eco-success" },
              ].map((item) => (
              ...
              {miniDevices.map((d) => (
                <div key={d.name} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/50">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">{d.power}</span>
                    <span className={`w-2 h-2 rounded-full ${
                      d.status === "active" ? "bg-eco-success" : d.status === "waste" ? "bg-destructive" : "bg-eco-warning"
                    }`} />
                  </div>
                </div>
              ))}
            ...
            {/* Alert */}
            <div className="mt-4 p-3 rounded-xl border border-eco-warning/30 bg-eco-warning/5 flex items-start gap-2.5">
              <Zap className="w-4 h-4 text-eco-warning mt-0.5 flex-shrink-0" />
              <div className="text-xs text-foreground">
                <span className="font-semibold">AI Alert:</span> TV standby detected overnight — could save 0.18 kWh daily.
              </div>
            </div>
          </div>
        </motion.div>
